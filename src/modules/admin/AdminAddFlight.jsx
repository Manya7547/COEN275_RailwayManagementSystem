...
const AdminAddTrain = (props) => {
    ...
    const [TrainIds, setFormIds] = useState("")
    if (TrainIds == "") {
      AdminService.getAllTrainsDetails().then(
        (ret) => {
          var arr = []
          ret.data.forEach(element => {
            arr.push(element.TrainNumber)
          });
          setFormIds(arr)
        }
      )
    ...
    const handleSaveClick = () =>{
        ...
        // check Train number existance
        if (TrainIds.includes(formData.TrainNumber)) {
          setSelectedValue("Train: "+ formData.TrainNumber + " already exists!")
          setNavigateBack(false)
          handleDialogBoxOpen()
          return
        }
        ...
        AdminService.saveTrainDetails(1, formData ).then((ret)=>{
            if(ret.data.includes("successfully")){
                //alert("Train details Added successfully");
                //props.history.push({pathname:`/admin`});
                setSelectedValue("Train details added successfully")
